> **English** · **[Русский](ru-Runtime)** · [← Wiki](Home)
...
Saving the script in the UI → Redis `script:updated` → runtime invalidates the venv cache → next run with new code **without restarting containers**.